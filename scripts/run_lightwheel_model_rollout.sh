...
run_args=(run --rm
  ...
  "$image"
  -lc
  "bash /host/scripts/ikea_live_container.sh --model-rollout --checkpoint '$checkpoint' --hdf5 '$hdf5' --demo '$demo' --max-frames '$frames' --action-projection '$projection' --output-dir '$output'"
)
